...
void compute_intersection_points() {  //called in run only. run() is used in Percolation_pbc.c once
  int index, tempi;
  int *flag;
...
int store_triangle(int NN)
{
  ...
  for(ii=0; ii<_ntrigs; ii++)
        {
          if(TriangleArea(_triangles[ii])>DBL_EPSILON) {NT++;}
        }
  ...
  if(_ntrigs>1)
    {
     ...
     int zero_tri=0;
      ...
      if(NN==NN_check){  sprintf(hname, "%strianles_iso_%lf_NN%d.dat", fout, rth,NN);
      fp=fopen(hname, "w");
      }
