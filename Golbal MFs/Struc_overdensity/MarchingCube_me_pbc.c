...
void compute_intersection_points() {  //called in run only. run() is used in main.c once
  ...
  //printf("sata:count = %d\n", index); //I forgot what it means, why it was required!!!
  free(flag);
}
...
int store_triangle(int NN)
{
	NT=0;
  int ii,jj=0;
  //double * tri_AREA; tri_AREA=(double *)malloc(_ntrigs*sizeof(double)); int tri_AREA_created=0; 
  double tri_AREA;
  for(ii=0; ii<_ntrigs; ii++)
        {
       	  tri_AREA=TriangleArea(_triangles[ii]); //tri_AREA_created=1;	
          if(tri_AREA>0.0) {NT++;}
        }
  ...
  if(_ntrigs>1)
    {
     ...
     zero_tri=0;
      ...
      if(NN==NN_check){  sprintf(hname, "%s_%d.dat", fout, NN);
      fp=fopen(hname, "w");
      }
      fp1=fopen("zero_tri.dat","w");
      for(ii=0; ii<_ntrigs; ii++)
        {
          tri_AREA=TriangleArea(_triangles[ii]);
          if(tri_AREA>0.0)
       ...
       //if (tri_AREA_created==1){free(tri_AREA);}
       return NT;
